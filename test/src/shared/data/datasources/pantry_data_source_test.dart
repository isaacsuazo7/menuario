...
void main() {
  group('PantryDataSourceImpl', () {
    ...
    test(
      'a saved quantity-tracked pantry item round-trips back with the '
      'same fields',
      () async {
        // Arrange
        ...
        const dto = PantryItemDTO.quantityTracked(
          category: 'cereal',
          presentation: PresentationDTO(type: 'package', yieldQty: 454),
          stock: QuantityDTO(
            value: 340,
            unitSymbol: 'g',
            unitDimension: 'mass',
    ...
    test(
      'a saved boolean-tracked pantry item round-trips back with the '
      'same fields',
      () async {
        // Arrange
        final dataSource = makeDataSource();
        const dto = PantryItemDTO.booleanTracked(
          category: 'condimento',
          presentation: PresentationDTO(type: 'loose'),
          haveIt: true,
        );
    ...
    test(
      'a pantry item written under uid A is not returned when scoped to '
      'uid B',
      () async {
        // Arrange
