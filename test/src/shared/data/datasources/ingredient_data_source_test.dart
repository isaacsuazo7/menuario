...
void main() {
  group('IngredientDataSourceImpl', () {
    ...
    test('an ingredient written under uid A is not returned when scoped to '
        'uid B', () async {
      // Arrange
    ...
    test('getById returns Left(Failure) instead of throwing when the '
        'document is missing a required field', () async {
      ...
      final dataSource = makeDataSource();
      await firestore.collection('users/uid-A/ingredients').doc('broken').set({
        'name': 'Avena',
      });
    ...
    test('getById loads an old-shape document (no measurementMode) without '
        'throwing, deriving a mode from its legacy fields', () async {
